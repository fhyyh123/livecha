...
public class WidgetConfigRepository {
    ...
    public Optional<WidgetConfigRow> findBySiteId(String siteId) {
        var sql = "select site_id, pre_chat_enabled, pre_chat_fields_json, theme_color, welcome_text, cookie_domain, cookie_samesite, widget_language, widget_phrases_json, pre_chat_message, pre_chat_name_label, pre_chat_email_label, pre_chat_name_required, pre_chat_email_required from widget_config where site_id = ? limit 1";
    ...
    public void upsert(
        ...
        var updateSql = "update widget_config set pre_chat_enabled = ?, pre_chat_fields_json = ?, theme_color = ?, welcome_text = ?, cookie_domain = ?, cookie_samesite = ?, widget_language = ?, widget_phrases_json = ?, pre_chat_message = ?, pre_chat_name_label = ?, pre_chat_email_label = ?, pre_chat_name_required = ?, pre_chat_email_required = ?, updated_at = current_timestamp where site_id = ?";
        var updated = jdbcTemplate.update(updateSql, preChatEnabled, preChatFieldsJson, themeColor, welcomeText, cookieDomain, cookieSameSite, widgetLanguage, widgetPhrasesJson, preChatMessage, preChatNameLabel, preChatEmailLabel, preChatNameRequired, preChatEmailRequired, siteId);
        if (updated > 0) return;

        var insertSql = "insert into widget_config(site_id, pre_chat_enabled, pre_chat_fields_json, theme_color, welcome_text, cookie_domain, cookie_samesite, widget_language, widget_phrases_json, pre_chat_message, pre_chat_name_label, pre_chat_email_label, pre_chat_name_required, pre_chat_email_required, created_at, updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?, ?, current_timestamp, current_timestamp)";
        jdbcTemplate.update(insertSql, siteId, preChatEnabled, preChatFieldsJson, themeColor, welcomeText, cookieDomain, cookieSameSite, widgetLanguage, widgetPhrasesJson, preChatMessage, preChatNameLabel, preChatEmailLabel, preChatNameRequired, preChatEmailRequired);
    }
}
