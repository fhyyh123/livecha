...
public class SiteRepository {
    ...
    public record SiteRow(String id, String tenantId, String status, boolean allowlistEnabled) {
    }

    public record SiteAdminRow(String id, String tenantId, String name, String publicKey, String status, boolean allowlistEnabled) {
    }
    ...
    public Optional<SiteRow> findByPublicKey(String publicKey) {
        var sql = "select id, tenant_id, status, allowlist_enabled from site where public_key = ? limit 1";
        var list = jdbcTemplate.query(sql, (rs, rowNum) -> new SiteRow(
                rs.getString("id"),
                rs.getString("tenant_id"),
            rs.getString("status"),
            rs.getBoolean("allowlist_enabled")
        ), publicKey);
        return list.stream().findFirst();
    }

    public Optional<SiteAdminRow> findById(String tenantId, String siteId) {
        var sql = "select id, tenant_id, name, public_key, status, allowlist_enabled from site where tenant_id = ? and id = ? limit 1";
    ...
    public java.util.List<SiteAdminRow> listByTenant(String tenantId) {
        var sql = "select id, tenant_id, name, public_key, status, allowlist_enabled from site where tenant_id = ? order by created_at desc";
    ...
    public SiteAdminRow create(String tenantId, String name, String publicKey) {
        ...
        return new SiteAdminRow(id, tenantId, siteName, publicKey.trim(), "active", true);
    }

    public boolean setAllowlistEnabled(String tenantId, String siteId, boolean enabled) {
        var sql = "update site set allowlist_enabled = ? where tenant_id = ? and id = ?";
        return jdbcTemplate.update(sql, enabled, tenantId, siteId) > 0;
    }
}
